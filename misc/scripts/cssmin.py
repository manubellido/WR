...
# -*- coding: utf-8 -*-
"""
    Based on: cssmin – A Python port of the YUI CSS compressor.
    (c) 2010 Zachary Voase. BSD Licensed.
"""
import os
...
ROOT_DIR = normpath(join(THIS_DIR, '..', '..', 'static', 'css'))
...
def needs_updating(name):
    ...
    name_min = name.replace('.css', '.min.css')
...
def main():
    ...
    print 'Concatenando CSS centrales > core.min.css'
    subprocess.call('rm ' + join(ROOT_DIR, 'core.min.css'), shell=True)
    cmd = ' '.join(['cat',
        join(ROOT_DIR, 'bootstrap.min.css'),
        join(ROOT_DIR, 'bootstrap-responsive.min.css'), 
        join(ROOT_DIR, 'font-awesome.min.css'), 
        join(ROOT_DIR, 'main.min.css'),
        '>',
        join(ROOT_DIR, 'core.min.css'),
    ])
